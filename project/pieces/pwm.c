//TODO: change to alternate port to free up spi master on port c
#include <avr/io.h>
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
...
void setup_gpio(); 
/*end of fun */
void tcc0_pwm_control(uint8_t ,uint16_t , float );
	...
	if (rampup!=0){
		...
		if ( rampup>(1<<6))
			{	
				...
				tcc0_pwm_control(20,pow(10,3),speed);
			}
	}
    }
...
void tcc0_pwm_control(uint8_t time, uint16_t t_scale, float duty_cycle){
